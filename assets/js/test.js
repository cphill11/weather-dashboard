...
// make 2nd api call https://api.openweathermap.org/data/2.5/onecall?lat={lat}&lon={lon}&exclude={part}&appid={API key}
  ...
  // fetch goes to encoded URL & retrieves data
  fetch(encoded).then(response => response.json()).then(data => {
      // do stuff w/ data
      console.log(data);
     // attempt to get 2nd API
     fetch(url)
      .then(response => response.json()).then(data => {
        console.log(data);

        const {coord: {lon, lat}} = data.city;
        console.log(lon, lat);
        ...
        displayForecastCurrentDay(data.daily[0]);
        //displayForecastFiveDay(data.daily);
      })
